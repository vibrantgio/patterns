...
// `levels` is every level there is, and therefore the whole set of
...
func TestOutlineColorClearsTheGraphicFloor(t *testing.T) {
	for _, sc := range []struct {
		...
		t.Run(sc.name, func(t *testing.T) {
			c := sc.colors
			for _, fill := range levels {
				edgeColor := outline.Color(c, c.SurfaceAt(fill.level))
				for _, plane := range levels {
...
// TestOutlineColorClearsTheFloorForEverySeed walks the same pairings over a
...
// may not.
func TestOutlineColorClearsTheFloorForEverySeed(t *testing.T) {
	worst := 99.0
